...
// ---------------------------------------------------------------------------
export async function toolGetWeather(city) {
    ...
    for (let i = 1; i <= 5; i++) {
        lines.push(`| ${w.daily.time[i]} | ${WMO_CODES[w.daily.weathercode[i]] ?? "?"} | ${w.daily.temperature_2m_max[i]}°C | ${w.daily.temperature_2m_min[i]}°C | ${w.daily.precipitation_sum[i]} mm | ${w.daily.windspeed_10m_max[i]} km/h |`);
    }
    lines.push("", `_Data source: Open-Meteo — updated hourly._`);
    return lines.join("\n");
}
export function toolSearchDestinations(travel_style, budget_tier, month, trip_duration_days, group_size) {
    const styles = travel_style.toLowerCase().split(",").map((s) => s.trim());
    const scored = [];
    for (const [name, d] of Object.entries(DESTINATION_DB)) {
        let score = 0;
        if (d.budget_tier.includes(budget_tier))
            score += 3;
        if (d.best_months.includes(month))
            score += 3;
        for (const s of styles)
            if (d.style.includes(s))
                score += 2;
        if (score > 0)
...
export function toolBuildItinerary(destination, duration_days, travel_style, start_date) {
    const dest = DESTINATION_DB[destination];
    const specificHighlights = dest ? dest.highlights : [];
    const genericDay = (day, dateLabel) => [
...
export function toolCompareTransport(origin, destination, group_size, travel_date) {
    const lines = [
...
export function toolCompareAccommodation(destination, duration_nights, group_size, travel_style) {
    const dest = DESTINATION_DB[destination];
...
export function toolEstimateBudget(destination, origin, duration_days, group_size, budget_tier) {
    const dest = DESTINATION_DB[destination];
    const tierKey = budget_tier === "mid-range" ? "mid_range" : budget_tier;
...
export function toolGetVisaInfo(destination, passport_nationality) {
    const dest = DESTINATION_DB[destination];
    const lines = [`## 🛂 Visa Information — ${destination}`, `_Passport: ${passport_nationality}_`, ""];
    if (dest)
        lines.push(`**Known guidance:** ${dest.visa_note}`, "");
    lines.push("**⚠️ Important:** Visa rules change frequently. Always verify with official sources.", "", "**Official resources:**", "- [IATA Travel Centre](https://www.iatatravelcentre.com)", "- [VisaHQ](https://www.visahq.com)", "- [Sherpa by Kayak](https://www.joinsherpa.com)", "", "**Document checklist:**", "- ✅ Passport valid 6+ months beyond return date", "- ✅ Return / onward ticket", "- ✅ Proof of accommodation", "- ✅ Proof of sufficient funds", "- ✅ Travel insurance with medical coverage");
    return lines.join("\n");
}
export function toolGetLocalTips(destination) {
    const dest = DESTINATION_DB[destination];
    const lines = [`## 💡 Local Tips — ${destination}`, ""];
    if (dest) {
        lines.push(`**Currency:** ${dest.currency}`);
    ...
    lines.push("**General tips:**", "- 💱 Use bank ATMs; avoid airport exchange for bulk conversions", "- 📶 Buy a local SIM on arrival for cheap data", "- 🚕 Use official taxis or Uber/Grab/Bolt", "- 🔌 Check plug type and voltage — carry a universal adapter", "- 📱 Download offline maps (Google Maps / Maps.me) before arrival", "- 🏛️ Respect dress codes at religious sites", "", "**Recommended apps:**", "| App | Purpose |", "|-----|---------|", "| Google Translate | Language + camera text |", "| Maps.me | Offline navigation |", "| XE Currency | Real-time exchange rates |", "| TripAdvisor | Reviews & attraction info |");
    return lines.join("\n");
}
export async function toolConvertCurrency(amount, from, to) {
    ...
    if (rate === undefined)
        throw new Error(`Target currency "${TO}" not found.`);
...
export function toolCheckTravelAlerts(destination, nationality) {
    const dest = DESTINATION_DB[destination];
    const lines = [
    ...
    if (dest)
        lines.push(`**Local safety context:** ${dest.safety}`, "");
    lines.push("**Official advisory portals:**", "| Country | Portal |", "|---------|--------|", "| 🇺🇸 USA | [travel.state.gov](https://travel.state.gov) |", "| 🇬🇧 UK | [gov.uk/foreign-travel-advice](https://www.gov.uk/foreign-travel-advice) |", "| 🇦🇺 Australia | [smartraveller.gov.au](https://www.smartraveller.gov.au) |", "| 🇨🇦 Canada | [travel.gc.ca](https://travel.gc.ca/travelling/advisories) |", "| 🌍 Others | Search '[your country] foreign travel advice' |", "", "**Before you go:**", "- Register your trip with your embassy", "- Save local emergency numbers: police, ambulance, fire", "- Share your itinerary with a trusted contact", "- Ensure travel insurance covers planned activities");
    return lines.join("\n");
}
