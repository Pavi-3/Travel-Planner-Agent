...
server.registerTool(
  ...
  async ({ city }) => {
    try { return ok(await toolGetWeather(city)); }
    catch (e) { return err(`Weather lookup failed: ${e instanceof Error ? e.message : String(e)}`); }
  }
);
...
server.registerTool(
  ...
  async ({ travel_style, budget_tier, month, trip_duration_days, group_size }) => {
    try { return ok(toolSearchDestinations(travel_style, budget_tier, month, trip_duration_days, group_size)); }
    catch (e) { return err(`Destination search failed: ${e instanceof Error ? e.message : String(e)}`); }
  }
);
...
server.registerTool(
  ...
  async ({ destination, duration_days, travel_style, start_date }) => {
    try { return ok(toolBuildItinerary(destination, duration_days, travel_style, start_date)); }
    catch (e) { return err(`Itinerary generation failed: ${e instanceof Error ? e.message : String(e)}`); }
  }
);
...
server.registerTool(
  ...
  async ({ amount, from_currency, to_currency }) => {
    try { return ok(await toolConvertCurrency(amount, from_currency, to_currency)); }
    catch (e) { return err(`Currency conversion failed: ${e instanceof Error ? e.message : String(e)}`); }
  }
);
