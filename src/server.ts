...
import {
  toolGetWeather,
  toolSearchDestinations,
  toolBuildItinerary,
  toolCompareTransport,
  toolCompareAccommodation,
  toolEstimateBudget,
  toolGetVisaInfo,
  toolGetLocalTips,
  toolConvertCurrency,
  toolCheckTravelAlerts,
  DESTINATION_DB,
} from "./tools.js";
...
// ---------------------------------------------------------------------------
// IAM token cache (avoid hammering the token endpoint)
// ---------------------------------------------------------------------------
...
async function getIAMToken(): Promise<string> {
  ...
  if (!res.ok) throw new Error(`IAM authentication failed: ${res.status} ${res.statusText}`);
...
async function discoverModel(): Promise<string> {
  ...
  try {
    ...
    const res = await fetch(
      `${WATSONX_URL}/ml/v1/foundation_model_specs?version=2024-05-31&limit=200`,
      { headers: { Authorization: `Bearer ${token}`, Accept: "application/json" } }
    );
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = (await res.json()) as { resources: Array<{ model_id: string }> };
    ...
    const anyGranite = data.resources.find(
      (m) => m.model_id.includes("granite") && m.model_id.includes("instruct")
    );
    if (anyGranite) {
...
// ---------------------------------------------------------------------------
// Tool detection — parse user message and auto-invoke structured tools
// ---------------------------------------------------------------------------

interface ToolResult { toolName: string; content: string; }

async function runAutoTools(message: string): Promise<ToolResult[]> {
  ...
  if (currencyMatch) {
    try {
      const content = await toolConvertCurrency(
        parseFloat(currencyMatch[1]),
        currencyMatch[2],
        currencyMatch[3]
      );
...
// ---------------------------------------------------------------------------
// Conversation history (in-memory, keyed by session ID)
// ---------------------------------------------------------------------------

interface Message { role: "user" | "assistant"; content: string; }
const sessions = new Map<string, Message[]>();

function buildConversationPrompt(history: Message[], newUserMessage: string): string {
  let prompt = "";
...
async function generateWithHistory(sessionId: string, userMessage: string): Promise<string> {
  const history = sessions.get(sessionId) ?? [];
  const token = await getIAMToken();

  const fullPrompt =
    `<|start_of_role|>system<|end_of_role|>${WANDERLOG_SYSTEM_PROMPT}<|end_of_text|>\n` +
...
app.post("/api/tool", async (req: Request, res: Response) => {
  try {
    ...
    switch (tool) {
      case "get_weather":
        result = await toolGetWeather(params.city as string);
        break;
      case "search_destinations":
        result = toolSearchDestinations(
          params.travel_style as string,
          params.budget_tier as "budget" | "mid-range" | "premium",
          params.month as number,
          params.trip_duration_days as number,
          params.group_size as number
        );
        break;
      case "build_itinerary":
        result = toolBuildItinerary(
          params.destination as string,
          params.duration_days as number,
          params.travel_style as string,
          params.start_date as string | undefined
        );
        break;
      case "compare_transport":
        result = toolCompareTransport(
          params.origin as string,
          params.destination as string,
          params.group_size as number,
          params.travel_date as string | undefined
        );
        break;
      case "compare_accommodation":
        result = toolCompareAccommodation(
          params.destination as string,
          params.duration_nights as number,
          params.group_size as number,
          params.travel_style as "budget" | "mid-range" | "premium"
        );
        break;
      case "estimate_budget":
        result = toolEstimateBudget(
          params.destination as string,
          params.origin as string,
          params.duration_days as number,
          params.group_size as number,
        ...
        break;
      case "get_visa_info":
        result = toolGetVisaInfo(params.destination as string, params.passport_nationality as string);
        break;
      case "get_local_tips":
        result = toolGetLocalTips(params.destination as string);
        break;
      case "convert_currency":
        result = await toolConvertCurrency(
          params.amount as number,
          params.from_currency as string,
          params.to_currency as string
        );
        break;
      case "check_travel_alerts":
        result = toolCheckTravelAlerts(params.destination as string, params.nationality as string);
        break;
      default:
...
app.get("/api/models", async (_req, res) => {
  try {
    ...
    const data = (await r.json()) as { resources: Array<{ model_id: string }> };
